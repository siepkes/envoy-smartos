...
Wheter you are on Triton or on SmartOS you need to create a native (`joyent` brand) container for the build. Steps below are performed on a container running the `base-64` image version `25.4.0` (`b933df4b-b8f4-4bef-ad1e-236a203044961`).
...
```
# pkgin -y install go124 ninja-build gcc13 git-base zip unzip openjdk21 libtool cmake automake ninja-build autoconf gmake python311 jq
```

Rust drives the Hickory DNS resolver and the dynamic-modules extensions. The port downloads the pinned Rust toolchain (1.88.0, the version Envoy expects) for `x86_64-unknown-illumos` from static.rust-lang.org — Rust ships tier-2 host tools for illumos — so no host Rust install is needed. `bazel/illumos-rules_rust.patch` teaches `rules_rust` the illumos triple. Only `cargo-bazel` (crate_universe's generator) has no illumos prebuilt, so it is bootstrapped from source with the downloaded `cargo`.

`aspect_bazel_lib` (a transitive Bazel dependency) provides `jq` and `yq` toolchains by downloading prebuilt binaries, but there are no illumos builds of those. The port therefore patches `aspect_bazel_lib` to use the host-installed `jq` and `yq` instead.  If it is not in pkgsrc, install it with the host Go:

```
$ go install github.com/mikefarah/yq/v4@v4.24.4   # YQ_VERSION in bazel/dependency_imports.bzl
```

Bazel will try to build the extensions that use Python (for example Kafka filter) for every Python version that is installed. Meaning you need to have the Python modules such as `py312-expat` installed for every installed Python version (for example `py311-expat` if Python 3.11 is also installed). When bumping Triton image version verify the package Python version. So beware that having other versions of Python installed in your build VM might complicate the build process. The same goes for Go; Having a more recent version or multiple versions of Go installed can lead to build issues.

Prepare the build environment:
```
$ git clone https://github.com/siepkes/envoy-smartos.git
$ cd envoy-smartos
$ git checkout smartos-v1.38.3
```

Build Envoy:
```
$ export PATH=$PATH:$HOME/go/bin # Needed to find 'yq'.
...
### Make Rust (dynamic modules) work

Everything related to Rust is disabled, which mostly affects dynamic modules. This is because the Rust code in Envoy requires Bazel's rules_rust, which does not have illumos support. Adding illumos support to rules_rust will require work (unclear how much) because we will have to bootstrap Bazel's crate_universe for illumos.
