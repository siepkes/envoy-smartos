...
Create a SmartOS container (`joyent` brand if your on Joyent's public cloud / Triton). Steps below are performed on a container running the `base-64` image version `20.4.0` (`1d05e788-5409-11eb-b12f-037bd7fee4ee`). 
...
```
# pkgin -y install go115 ninja-build gcc9 git-base zip unzip openjdk11 libtool cmake automake ninja-build autoconf gmake python38 py38-expat
```

Bazel will try to build the extensions that use Python (for example Kafka filter) for every Python version that is installed. Meaning you need to have the Python modules such as `py38-expat` installed for every installed Python version (for example `py37-expat` if Python 3.7 is also installed). When bumping Triton image version verify the package Python version. So beware that having other versions of Python installed in your build VM might complicate the build process. The same goes for Go; Having a more recent version or multiple versions of Go installed can lead to build issues.
...
We currently disable WASM and the default V8 webassembly runtime with the `--//source/extensions/wasm_runtime/v8:enabled=false` flag when building. Reason for this is that the V8 WASM runtime currently doesn't build on Illumos. Envoy can be configured to use a different WASM runtime but for now WASM is just disabled.
